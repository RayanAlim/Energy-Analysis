...
# Read the CSV file, skip first 4 rows and check the structure
gdp_data_raw <- read.csv("../data/raw_data/Global_GDP_Growth.csv", skip=4, check.names = FALSE)

# Clean and normalize column names
colnames(gdp_data_raw) <- make.names(colnames(gdp_data_raw), unique = TRUE)

# Specify columns to keep: country details and years 1992 to 2022
...
gdp_data_long <- pivot_longer(gdp_data_selected,
                              cols = starts_with("X"),
                              names_to = "Year",
                              values_to = "GDP_growth",
                              names_prefix = "X")  # Remove the 'X' prefix in year column
...
# Read the CSV file
energy_data <- read_csv("../data/raw_data/primary-energy-cons.csv",show_col_types = FALSE)
...
combined_data <- full_join(energy_data_filtered, gdp_data_filtered, by = c("Entity", "Code", "Year"))

# Define the list of income categories
income_categories <- c("High-income countries", 
...
income_data_subset <- income_data_subset %>%
  mutate(Entity = ifelse(Entity %in% names(income_region_mapping), income_region_mapping[Entity], Entity))
...
income_data_subset$Entity <- tolower(income_data_subset$Entity)  # Make entity names lowercase for consistency
income_data_subset$Average_GDP_Growth[income_data_subset$Average_GDP_Growth == -Inf] <- NA  # Replace -Inf with NA
...
income_data_subset <- income_data_subset %>%
  mutate(Standardized_Entity = case_when(
...
aggregated_data <- income_data_subset %>%
  ...
  summarise(Total_Energy_Consumption_TWh = sum(`Primary energy consumption (TWh)`, na.rm = TRUE),
            Average_GDP_Growth = mean(GDP_growth, na.rm = TRUE),
            .groups = 'drop')
...
aggregated_data %>%
  filter(is.na(Total_Energy_Consumption_TWh) | is.na(Average_GDP_Growth))
...
entities_to_exclude <- c(
  "Africa", "Africa (EI)", "Africa (EIA)", "Antarctica", "Asia", 
  "Asia & Oceania (EIA)", "Asia Pacific (EI)", "Australia and New Zealand (EIA)", 
  "CIS (EI)", "Central & South America (EIA)", "Central America (EI)", 
  "Eastern Africa (EI)", "Eurasia (EIA)", "Europe", "Europe (EI)", 
  "Europe (EIA)", "European Union (27)", "High-income countries", 
  "Low-income countries", "Lower-middle-income countries", "Middle Africa (EI)", 
  "Middle East (EI)", "Middle East (EIA)", "Non-OECD (EI)", "Non-OECD (EIA)", 
  "Non-OPEC (EIA)", "North America", "North America (EI)", "OECD (EI)", 
  "OECD (EIA)", "OPEC (EIA)", "Oceania", "Persian Gulf (EIA)", "South America", 
  "South and Central America (EI)", "U.S. Pacific Islands (EIA)", "U.S. Territories (EIA)", 
  "Upper-middle-income countries", "Wake Island (EIA)", "Western Africa (EI)", 
  ...
  "Sub-Saharan Africa (excluding high income)", "High income", "Low & middle income", 
  "Low income", "Lower middle income", "Middle income", "Upper middle income", "Africa Eastern and Southern",
  "Africa Western and Central", "Early-demographic dividend", "IBRD only", "IDA & IBRD total",
  "IDA total", "IDA blend", "IDA only", "Not classified", "Sub-Saharan Africa (IDA & IBRD countries)", 
  ...
  "East Asia & Pacific (IDA & IBRD countries)", "Post-demographic dividend", "Pre-demographic dividend",
...
#### Save data ####
write.csv(filtered_Country_combined_data, "../data/analysis_data/countries_energy_GDP_data.csv", row.names = FALSE)
write.csv(aggregated_data, "../data/analysis_data/income_group_energy_GDP_data.csv", row.names = FALSE)
